...
sub cnstat{
	my ($chrid,$cutlen,$edgeinfo,$seqcover,$refcover,$loxpregion,$lastindex) = @_;
  my @cns;
  my %testee_depth;
  my %copynum;
  my %ref_depth;
  ...
  foreach (keys %{$edgeinfo}){
    ...
    foreach (@regidx){
      ...
      my $starsite = ${${$loxpregion}{$_}}[1];
      my $endsite = ${${$loxpregion}{$_}}[2];  
      
      my $average_depth_testee = averdep($chrid,$starsite,$endsite,\%$seqcover);
      my $average_depth_ref = averdep($chrid,$starsite,$endsite,\%$refcover);
      ...
      if($idx!=1&&$idx!= $lastindex &&($endsite-$starsite+1)>=2*$cutlen){
        ...
        foreach(1..$n-1){    	
          ...
          $average_depth_testee = averdep($chrid,$starsitenew,$endsitenew,\%$seqcover);
          $average_depth_ref = averdep($chrid,$starsitenew,$endsitenew,\%$refcover);
       ...
       $key = "$idx".".$n";
       $average_depth_testee = averdep($chrid,$starsitenew,$endsite,\%$seqcover);
       $average_depth_ref = averdep($chrid,$starsitenew,$endsite,\%$refcover);
       $testee_depth{$key} = $average_depth_testee;
       $ref_depth{$key} = $average_depth_ref;
       $copynum{$key} = $edge_info[1];        
       $regidxedg{$key} = $edge_info[0];
