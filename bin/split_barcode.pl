...
if(@ARGV != 5){
    print "\n";
    print "Example: perl $0.pl barcode.list r1.fq.gz r2.fq.gz filename outdir \n";
    print "\n";
    exit(1);
}
...
open OUT1,"| gzip >$ARGV[4]/$ARGV[3].1.fq.gz" or die "Can't create $ARGV[4]/$ARGV[3].1.fq.gz\n";
...
open OUT2,"| gzip > $ARGV[4]/$ARGV[3].2.fq.gz" or die "Can't create $ARGV[4]/$ARGV[3].2.fq.gz\n";
